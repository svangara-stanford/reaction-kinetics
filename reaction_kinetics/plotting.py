...
def _field_axis_labels(analysis_field: str) -> Tuple[str, str, str]:
    """Labels for (concentration/state, rate, std of state) in descriptive plots."""
    if str(analysis_field).lower() == "x_li":
        return ("x_li", "dx_li/dt", "x_li")
    return ("SOC", "dc/dt", "SOC")
...
def plot_particle_soc_curves(
    ...
    """Overlay particle-average state (SOC or x_li) vs time for all particles."""
    c_lab, _, _ = _field_axis_labels(analysis_field)
    fig, ax = plt.subplots()
    for ep in episodes:
        mean_soc = []
    ...
    ax.set_ylabel(f"Mean {c_lab}")
    ax.legend(loc="best", fontsize=8)
    ax.set_title(f"Particle-average {c_lab} vs time")
...
def plot_dc_dt_vs_c(
    ...
    """Scatter rate vs state split by rising vs falling voltage (T-1 midpoints)."""
    c_lab, r_lab, _ = _field_axis_labels(analysis_field)
    c_rising, dc_rising = [], []
    ...
    for ax, all_c, all_dc, title in [
        ...
        if len(all_c) == 0:
            ax.set_title(title)
            ax.set_xlabel(f"{c_lab} (midpoint)")
            ax.set_ylabel(r_lab)
...
def plot_heterogeneity_vs_time(
    ...
    """Within-particle std of state vs time (and global mean)."""
    _, _, std_lab = _field_axis_labels(analysis_field)
    fig, ax = plt.subplots()
    ...
    ax.set_xlabel("Time (h)")
    ax.set_ylabel(f"Std {std_lab} within particle")
    ax.legend(loc="best", fontsize=8)
    ax.set_title(f"Heterogeneity (std {std_lab}) vs time")
...
def plot_valid_coverage(
    ...
    ax_soc_union.set_ylabel(f"Frac valid {c_lab}")
    ax_soc_union.legend(loc="best", fontsize=7)
    ax_soc_union.set_title(f"{c_lab} coverage (over union)")
    ax_soc_cond.set_ylabel(f"Frac valid {c_lab}")
    ax_soc_cond.legend(loc="best", fontsize=7)
    ax_soc_cond.set_title(f"{c_lab} coverage (within mask at t)")
    ax_dc_union.set_ylabel(f"Frac valid {r_lab}")
    ax_dc_union.legend(loc="best", fontsize=7)
    ax_dc_union.set_title(f"{r_lab} coverage (over union)")
    ax_dc_cond.set_ylabel(f"Frac valid {r_lab}")
    ax_dc_cond.legend(loc="best", fontsize=7)
    ax_dc_cond.set_title(f"{r_lab} coverage (within active pair)")
...
def plot_persistent_norm_rate_maps(
    ...
    """Plot time-mean persistent normalized rate (signed rate / mean|rate| per t) per particle.
    Color scale is symmetric about 0. Optional global scale across all panels."""
    _, r_lab, _ = _field_axis_labels(analysis_field)
    ...
    fig.suptitle(
        f"Persistent norm. rate: signed {r_lab} / mean|{r_lab}| per t",
        fontsize=10,
        y=1.02,
    )
...
def plot_boundary_vs_interior(
    ...
    """Mean state and mean |rate| vs time for boundary vs interior."""
    c_lab, r_lab, _ = _field_axis_labels(analysis_field)
    ...
    ax_soc.set_ylabel(f"Mean {c_lab}")
    ax_soc.set_xlabel("Time (h)")
    ax_soc.set_title(f"Boundary vs interior: mean {c_lab} vs time")
    ax_soc.legend(loc="best", fontsize=7)
    ax_rate.set_ylabel(f"Mean |{r_lab}|")
    ax_rate.set_xlabel("Time (h)")
    ax_rate.set_title(f"Boundary vs interior: mean |{r_lab}| vs time")
...
def plot_per_particle_dc_dt_vs_soc(
    ...
    """Per-particle: 4 panels — rate vs state and |rate| vs state, each split rising/falling."""
    c_lab, r_lab, _ = _field_axis_labels(analysis_field)
    ...
    for ax, all_c, all_dc, title, use_abs in [
        (ax_rise_dc, c_rise, dc_rise, f"Rising: {r_lab} vs {c_lab}", False),
        (ax_fall_dc, c_fall, dc_fall, f"Falling: {r_lab} vs {c_lab}", False),
        (ax_rise_abs, c_rise, np.abs(dc_rise), f"Rising: |{r_lab}| vs {c_lab}", True),
        (ax_fall_abs, c_fall, np.abs(dc_fall), f"Falling: |{r_lab}| vs {c_lab}", True),
    ]:
        if len(all_c) == 0:
            ax.set_title(title)
            ax.set_xlabel(f"{c_lab} (midpoint)")
            ax.set_ylabel(f"|{r_lab}|" if use_abs else r_lab)
            ax.axhline(0, color="gray", ls="--")
            continue
        if len(all_c) > 2000:
...
def plot_per_particle_mean_summaries(
    ...
    """Per-particle: mean state vs t, mean rate vs t, mean |rate| vs t, mean |rate| vs mean state."""
    c_lab, r_lab, _ = _field_axis_labels(analysis_field)
    ...
    ax4.set_xlabel(f"Mean {c_lab} (midpoint)")
    ax4.set_ylabel(f"Mean |{r_lab}|")
    ax4.set_title(f"Mean |{r_lab}| vs mean {c_lab}")
...
def plot_per_particle_boundary_vs_interior(
    ...
    """Per-particle: boundary vs interior state and |rate| vs time."""
    c_lab, r_lab, _ = _field_axis_labels(analysis_field)
...
def plot_per_particle_diagnostics(
    ...
    for i, (ep, rate, p) in enumerate(zip(episodes, rate_maps_list, summary.per_particle)):
        ...
        paths.append(
            plot_per_particle_dc_dt_vs_soc(ep, rate, particle_dir, pid, analysis_field=analysis_field)
        )
        p1 = plot_per_particle_mean_summaries(p, particle_dir, pid, analysis_field=analysis_field)
        if p1:
            paths.append(p1)
        paths.append(
            plot_per_particle_boundary_vs_interior(p, particle_dir, pid, analysis_field=analysis_field)
        )
...
def plot_all(
    ...
    paths = []
    paths.append(plot_particle_soc_curves(episodes, summary, out_dir, analysis_field=analysis_field))
    paths.append(plot_dc_dt_vs_c(episodes, rate_maps_list, out_dir, analysis_field=analysis_field))
    paths.append(plot_heterogeneity_vs_time(summary, out_dir, analysis_field=analysis_field))
    paths.append(plot_valid_coverage(summary, out_dir, analysis_field=analysis_field))
    p1 = plot_persistent_norm_rate_maps(episodes, summary, out_dir, analysis_field=analysis_field)
    if p1:
        paths.append(p1)
    ...
    if p3:
        paths.append(p3)
    paths.append(plot_boundary_vs_interior(summary, out_dir, analysis_field=analysis_field))
    paths.extend(
        plot_per_particle_diagnostics(
            episodes, rate_maps_list, summary, out_dir, analysis_field=analysis_field
        )
    )
    return paths
...
def plot_additional_xli_current_maps(
    episodes: List[ParticleEpisode],
    time_mid_s: np.ndarray,
    scan_region_maps: Dict[str, np.ndarray],
    out_dir: str,
) -> List[str]:
    """Additional first-pass x_li/current figures; does not alter legacy plots."""
    paths: List[str] = []
    out = _ensure_dir(out_dir)

    # Particle-mean x_li vs time (T)
    fig, ax = plt.subplots()
    for ep in episodes:
        ...
        if x_li is None:
            continue
        mean_x_li = []
        for t in range(x_li.shape[0]):
            vals = x_li[t][ep.valid_mask_tyx[t]]
            vals = vals[np.isfinite(vals)]
            mean_x_li.append(float(np.mean(vals)) if len(vals) else np.nan)
        ax.plot(ep.time_s / 3600.0, mean_x_li, label=f"P{ep.particle_id}", alpha=0.8)
    ax.set_xlabel("Time (h) [T]")
    ax.set_ylabel("Mean x_li")
    ax.set_title("Particle-average lithium stoichiometry vs time")
    ax.legend(loc="best", fontsize=8)
    p = out / "x_li_vs_time_particles_T.png"
    fig.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    paths.append(str(p))

    # dx_li_dt vs x_li scatter (T-1 midpoint)
    ...
    for ep in episodes:
        ...
        if x_li is None or dx_li_dt is None:
            continue
        x_mid = (x_li[:-1] + x_li[1:]) / 2.0
        for t in range(dx_li_dt.shape[0]):
            v = np.isfinite(x_mid[t]) & np.isfinite(dx_li_dt[t]) & ep.valid_mask_tyx[t] & ep.valid_mask_tyx[t + 1]
            if np.any(v):
                x_all.append(x_mid[t][v].ravel())
                y_all.append(dx_li_dt[t][v].ravel())
    fig, ax = plt.subplots(figsize=(7, 5))
    if x_all:
        xx = np.concatenate(x_all)
        yy = np.concatenate(y_all)
        if len(xx) > 4000:
            idx = np.random.choice(len(xx), 4000, replace=False)
            xx = xx[idx]
            yy = yy[idx]
        ax.scatter(xx, yy, s=5, alpha=0.2)
    ax.set_xlabel("x_li midpoint [T-1]")
    ax.set_ylabel("dx_li/dt")
    ax.set_title("dx_li/dt vs x_li (scan pixels)")
    ax.axhline(0, color="gray", ls="--")
    p = out / "dx_li_dt_vs_x_li_Tminus1.png"
    fig.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    paths.append(str(p))

    # Scan-region-normalized current-density proxy (not full-cathode absolute j)
    j_tyx = scan_region_maps.get("scan_region_normalized_current_density_proxy_a_per_cm2_tyx")
    if j_tyx is not None and j_tyx.ndim == 3 and j_tyx.shape[0] > 0:
        n_show = min(6, j_tyx.shape[0])
        picks = np.linspace(0, j_tyx.shape[0] - 1, n_show).astype(int)
        ncol = min(3, n_show)
        nrow = (n_show + ncol - 1) // ncol
        fig, axes = plt.subplots(nrow, ncol, figsize=(4 * ncol, 4 * nrow))
        axes = np.array([axes]).reshape(-1) if n_show == 1 else np.array(axes).reshape(-1)
        finite = j_tyx[np.isfinite(j_tyx)]
        vmax = float(np.nanpercentile(finite, 98)) if len(finite) else 1.0
        vmax = vmax if np.isfinite(vmax) and vmax > 0 else 1.0
        for i, t in enumerate(picks):
            ax = axes[i]
            im = ax.imshow(j_tyx[t], cmap="viridis", vmin=0.0, vmax=vmax)
            label_t = time_mid_s[t] / 3600.0 if t < len(time_mid_s) else np.nan
            ax.set_title(f"t_mid={label_t:.2f} h [T-1]\n(Raman scan-region proxy)")
            plt.colorbar(
                im,
                ax=ax,
                label="current-density proxy (A/cm^2, scan-region-normalized)",
            )
        for j in range(n_show, len(axes)):
            axes[j].set_visible(False)
        fig.suptitle(
            "Raman-weighted scan-region-normalized current-density proxy (not full cathode j)",
            fontsize=10,
            y=1.02,
        )
        p = out / "scan_region_normalized_current_density_proxy_maps_panel_Tminus1.png"
        fig.tight_layout()
        fig.savefig(p, dpi=150, bbox_inches="tight")
        plt.close()
        paths.append(str(p))
...
def plot_current_share_xli_diagnostics(
    particle_ids: List[int],
    mask_union_by_id: Dict[int, np.ndarray],
    share_by_pid: Dict[int, np.ndarray],
    scan_region_maps: Dict[str, Any],
    time_mid_s: np.ndarray,
    out_dir: str,
) -> List[str]:
    """
    Diagnostic figures for exclusive current-share aggregation and overlap audit.
    """
    paths: List[str] = []
    out = _ensure_dir(out_dir)
    partition_invalid = scan_region_maps["partition_invalid_t"]

    # 1) Sum of retained-particle shares vs time (should stay <= 1)
    Tm1 = len(time_mid_s)
    sum_shares = np.full(Tm1, np.nan, dtype=float)
    for t in range(Tm1):
        if partition_invalid[t]:
            continue
        sum_shares[t] = float(
            np.sum([share_by_pid[pid][t] for pid in particle_ids])
        )
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(time_mid_s / 3600.0, sum_shares, "b-", lw=1.5, label="sum of particle shares")
    ax.axhline(1.0, color="gray", ls="--", label="reference = 1")
    ax.set_xlabel("Time (h) [T-1 midpoint]")
    ax.set_ylabel("Sum of exclusive particle shares")
    ax.set_title(
        "Retained particle current-share sum vs time\n(exclusive lowest particle_id; scan-region weights)"
    )
    ax.legend(loc="best", fontsize=8)
    ax.set_ylim(0.0, max(1.05, float(np.nanmax(sum_shares)) * 1.05 if np.any(np.isfinite(sum_shares)) else 1.05))
    p1 = out / "retained_particle_current_share_sum_vs_time.png"
    fig.tight_layout()
    fig.savefig(p1, dpi=150, bbox_inches="tight")
    plt.close()
    paths.append(str(p1))

    # 2) Pairwise mask overlap heatmap (union masks)
    n = len(particle_ids)
    mat = np.zeros((n, n), dtype=float)
    for i, pi in enumerate(particle_ids):
        for j, pj in enumerate(particle_ids):
            if i == j:
                mat[i, j] = np.nan
            elif i < j:
                mi = mask_union_by_id[pi].astype(bool)
                mj = mask_union_by_id[pj].astype(bool)
                mat[i, j] = mat[j, i] = float(np.sum(mi & mj))
    fig, ax = plt.subplots(figsize=(6, 5))
    im = ax.imshow(mat, cmap="YlOrRd")
    ax.set_xticks(range(n))
    ax.set_yticks(range(n))
    ax.set_xticklabels([str(p) for p in particle_ids])
    ax.set_yticklabels([str(p) for p in particle_ids])
    ax.set_xlabel("particle_id")
    ax.set_ylabel("particle_id")
    ax.set_title("Pairwise union-mask overlap (pixels)\ndiagonal blank; exclusive rule uses min id")
    plt.colorbar(im, ax=ax, label="overlap pixel count")
    p2 = out / "particle_overlap_diagnostic.png"
    fig.tight_layout()
    fig.savefig(p2, dpi=150, bbox_inches="tight")
    plt.close()
    paths.append(str(p2))

    # 3) Uncovered weight fraction vs time
    uncovered = np.full(Tm1, np.nan, dtype=float)
    for t in range(Tm1):
        if partition_invalid[t] or not np.isfinite(sum_shares[t]):
            continue
        uncovered[t] = float(1.0 - sum_shares[t])
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(time_mid_s / 3600.0, uncovered, color="C1", lw=1.5)
    ax.set_xlabel("Time (h) [T-1 midpoint]")
    ax.set_ylabel("Uncovered weight fraction")
    ax.set_title(
        "Fraction of scan-region weight not on retained particles\n(1 − sum exclusive shares)"
    )
    ax.set_ylim(0.0, 1.0)
    p3 = out / "uncovered_weight_fraction_vs_time.png"
    fig.tight_layout()
...
def plot_weight_region_breakdown_vs_time(
    time_mid_s: np.ndarray,
    weight_df: pd.DataFrame,
    out_dir: str,
) -> str:
    """Fraction of scan-region relative weight on retained vs non-retained valid pixels."""
    out = _ensure_dir(out_dir)
    hours = np.asarray(time_mid_s, dtype=float) / 3600.0
    fr = weight_df["fraction_on_retained_particles"].astype(float).to_numpy()
    fn = weight_df["fraction_on_nonretained_valid_pixels"].astype(float).to_numpy()
    pinv = weight_df["partition_invalid_frame"].to_numpy()
    fr = np.where(pinv, np.nan, fr)
    fn = np.where(pinv, np.nan, fn)
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(hours, fr, "b-", lw=1.5, label="fraction on retained particles")
    ax.plot(hours, fn, "C1-", lw=1.5, label="fraction on non-retained valid pixels")
    ax.set_xlabel("Time (h) [T-1 midpoint]")
    ax.set_ylabel("Fraction of total weight")
    ax.set_title(
        "Scan-region relative weight split by retained vs non-retained valid pixels\n"
        "(partition-invalid frames omitted)"
    )
    ax.legend(loc="best", fontsize=8)
    ax.set_ylim(0.0, 1.05)
    p = out / "weight_region_breakdown_vs_time.png"
    fig.tight_layout()
    fig.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    return str(p)
...
def plot_area_vs_weight_comparison(
    time_mid_s: np.ndarray,
    area_df: pd.DataFrame,
    out_dir: str,
) -> str:
    """Retained area vs weight fraction and per-unit-area weight ratios."""
    out = _ensure_dir(out_dir)
    hours = np.asarray(time_mid_s, dtype=float) / 3600.0
    pinv = area_df["partition_invalid_frame"].to_numpy()
    raf = area_df["retained_area_fraction"].astype(float).to_numpy()
    rwf = area_df["retained_weight_fraction"].astype(float).to_numpy()
    rwa = area_df["retained_weight_per_unit_area"].astype(float).to_numpy()
    nwa = area_df["nonretained_weight_per_unit_area"].astype(float).to_numpy()
    raf = np.where(pinv, np.nan, raf)
    rwf = np.where(pinv, np.nan, rwf)
    rwa = np.where(pinv, np.nan, rwa)
    nwa = np.where(pinv, np.nan, nwa)

    fig, axes = plt.subplots(2, 1, figsize=(8, 4.5), sharex=True)
    axes[0].plot(hours, raf, "b-", lw=1.5, label="retained area fraction")
    axes[0].plot(hours, rwf, "C1-", lw=1.5, label="retained weight fraction")
    axes[0].axhline(1.0, color="gray", ls=":", lw=0.8)
    axes[0].set_ylabel("Fraction")
    axes[0].set_title("Retained: area share vs weight share (uniform ⇒ overlap)")
    axes[0].legend(loc="best", fontsize=7)
    axes[0].set_ylim(0.0, 1.05)

    axes[1].plot(hours, rwa, "b-", lw=1.5, label="retained: weight / area")
    axes[1].plot(hours, nwa, "C1-", lw=1.5, label="non-retained: weight / area")
    axes[1].axhline(1.0, color="gray", ls=":", lw=0.8)
    axes[1].set_xlabel("Time (h) [T-1 midpoint]")
    axes[1].set_ylabel("Weight / area (uniform baseline = 1)")
    axes[1].set_title("Relative enrichment vs uniform density over each region")
    axes[1].legend(loc="best", fontsize=7)

    p = out / "area_vs_weight_comparison.png"
    fig.tight_layout()
    fig.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    return str(p)
...
def plot_boundary_weight_vs_interior(
    boundary_df: pd.DataFrame,
    out_dir: str,
) -> str:
    """Grouped bars: mean relative weight on boundary vs interior per particle."""
    out = _ensure_dir(out_dir)
    if boundary_df.empty:
        fig, ax = plt.subplots(figsize=(6, 3))
        ax.text(0.5, 0.5, "No boundary weight data", ha="center", va="center")
        p = out / "boundary_weight_vs_interior.png"
        fig.savefig(p, dpi=150, bbox_inches="tight")
        plt.close()
        return str(p)

    pids = boundary_df["particle_id"].astype(int).tolist()
    mb = boundary_df["mean_relative_weight_boundary"].astype(float).to_numpy()
    mi = boundary_df["mean_relative_weight_interior"].astype(float).to_numpy()
    x = np.arange(len(pids))
    w = 0.35
    fig, ax = plt.subplots(figsize=(max(6, 0.6 * len(pids)), 4))
    ax.bar(x - w / 2, mb, width=w, label="boundary", color="C0")
    ax.bar(x + w / 2, mi, width=w, label="interior", color="C1")
    ax.set_xticks(x)
    ax.set_xticklabels([str(p) for p in pids])
    ax.set_xlabel("particle_id")
    ax.set_ylabel("Mean relative current weight (scan region)")
    ax.set_title(
        "Retained particles: mean relative weight on boundary vs interior\n"
        "(full-field mask; valid-partition timesteps)"
    )
    ax.legend(loc="best", fontsize=8)
    p = out / "boundary_weight_vs_interior.png"
    fig.tight_layout()
